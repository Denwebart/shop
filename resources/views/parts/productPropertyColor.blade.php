...
@if(count($product->propertyColor))
    ...
    <ul class="options-swatch options-swatch--color options-swatch--lg">
        @foreach($product->propertyColor as $color)
            <li class="add-to-cart__color tooltip-link @if(Request::get($color->property_title) == $color->value || count($product->propertyColor) == 1) active @endif" title="{{ $color->value }}" data-value="{{ $color->value }}">
