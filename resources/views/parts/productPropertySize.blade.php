...
@if(count($product->propertySize))
    ...
    <ul class="options-swatch options-swatch--size options-swatch--lg">
        ...
        <li class="add-to-cart__size tooltip-link @if(Request::get($size->property_title) == $size->value || count($product->propertySize) == 1) active @endif" title="{{ $size->value }} @if($size->additional_value) ({{ $size->additional_value }}) @endif" data-value="{{ $size->value }}">
