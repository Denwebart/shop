...
return [
    ...
    */
    ...
    'image'                => 'В поле можно загрузить только изображение.',
    ...
    */
    ...
    'custom' => [
        'alias' => [
            'regex' => 'Поле может содержать только латинские символы, цифры и знаки дефиса.',
            'unique' => 'Alias должен быть уникальным. Такой alias уже существует.',
        ],
	    'vendor_code' => [
		    'regex' => 'Поле может содержать только латинские и кириллические буквы, цифры и знаки дефиса.',
	    ],
	    'price' => [
		    'numeric' => 'В поле можно вводить только цифры без пробелов.',
	    ],
	    'title' => [
		    'required_without' => 'Поле обязательно для заполнения, если не указан заголовок меню.',
	    ],
	    'menu_title' => [
		    'required_without' => 'Поле обязательно для заполнения, если не указан заголовок.',
	    ],
	    'password' => [
	    ...
	    'login' => [
		    'unique' => 'Такой логин уже существует. Придумайте другой.',
		    'regex' => 'Поле может содержать только латинские буквы, цифры, дефисы и знаки нижнего подчёркивания.',
	    ],
	    'email' => [
		    'unique' => 'Пользователь с таким электронным адресом уже зарегистрирован.',
	    ],
	    'firstname' => [
		    'regex' => 'Поле может содержать только латинские и кириллические буквы, дефисы и пробелы.',
	    ],
	    'lastname' => [
		    'regex' => 'Поле может содержать только латинские и кириллические буквы, дефисы и пробелы.',
	    ],
	    'user_name' => [
		    'required_without' => 'Это поле обязательно для заполнения.',
	    ],
	    'user_email' => [
		    'required_without' => 'Это поле обязательно для заполнения.',
	    ],
	    'category_id' => [
		    'not_in' => 'Выбор категории обязателен.',
	    ],
	    'type' => [
		    'between' => 'Неверное значение.',
	    ],
	    'role' => [
		    'between' => 'Выбор прав пользователя обязателен.',
	    ],
    ],
